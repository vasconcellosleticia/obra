...
const EditFiscalizacaoScreen = ({ route, navigation }) => {
  const { fiscalizacao } = route.params
  const { updateFiscalizacao, loading } = useObra()
  const [selectedImage, setSelectedImage] = useState(fiscalizacao.foto)
  const [selectedLocation, setSelectedLocation] = useState(fiscalizacao.localizacao)
  ...
  const {
    ...
    defaultValues: {
      data: new Date(fiscalizacao.data),
      status: fiscalizacao.status,
      observacoes: fiscalizacao.observacoes,
      fiscal: {
        nome: fiscalizacao.fiscal?.nome || "",
        registro: fiscalizacao.fiscal?.registro || "",
      },
      temperatura: fiscalizacao.temperatura?.toString() || "",
      condicaoClimatica: fiscalizacao.condicaoClimatica || "Ensolarado",
      nivelRisco: fiscalizacao.nivelRisco || "Baixo",
    },
  })
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await updateFiscalizacao(fiscalizacao._id, fiscalizacaoData)

      Toast.show({
        type: "success",
        text1: "Sucesso",
        text2: "Fiscalização atualizada com sucesso",
      })
      ...
      Toast.show({
        type: "error",
        text1: "Erro",
        text2: error.message || "Não foi possível atualizar a fiscalização",
      })
    }
  }
...
export default EditFiscalizacaoScreen
